...
describe('edit-post-controller', () => {
  ...
  it('should not be able to edit post if user is not authenticated', async () => {
    await request(app.getHttpServer()).put('/post').send().expect(401)
  })

  it('should be able to edit a post with success', async () => {
    await request(app.getHttpServer())
      .put('/post')
      .send({
        id: post.id,
